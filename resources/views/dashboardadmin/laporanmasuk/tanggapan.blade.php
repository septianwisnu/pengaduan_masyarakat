                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-body card-body-tanggapan mt-4">
                                <a href="/laporan_masuk" class="btn btn-secondary btn-secondary-tanggapan mb-3">&larr;
                                    Kembali</a>
                                <form action="/update_tanggapan/{{ $pengaduans->id }}" method="POST">
                                    ...
                                    <div class="row gy-4">
                                        ...
                                        <div class="col-12">
                                            ...
                                            <select class="form-control" name="status" required>
                                                <option value="ditolak"
                                                    {{ $pengaduans->status == 'ditolak' ? 'selected' : '' }}>ditolak
                                                </option>
                                                <option value="ditunda"
                                                    {{ $pengaduans->status == 'ditunda' ? 'selected' : '' }}>
                                                    Ditunda</option>
                                                <option value="diproses"
                                                    {{ $pengaduans->status == 'diproses' ? 'selected' : '' }}>Diproses
                                                </option>
                                                <option value="selesai"
                                                    {{ $pengaduans->status == 'selesai' ? 'selected' : '' }}>Selesai
                                                </option>
                                            </select>
                                            @error('status')
                                                <div class="text-danger">{{ $message }}</div>
                                        ...
                                        <div class="col-12 text-center">
                                            <button type="submit" class="btn btn-primary">Kirim Tanggapan & Perbarui
                                                Status</button>
                                        </div>
                                    </div>
                                </form>
